...
if __name__ == "__main__":
    ...
    lr1 = tf.get_variable(
        'lr1', shape=[], trainable=False,
        initializer=tf.constant_initializer(4e-4))
    lr2 = tf.get_variable(
        'lr2', shape=[], trainable=False,
        initializer=tf.constant_initializer(1e-4))

    d_optimizer = tf.train.AdamOptimizer(lr1, beta1=0.5, beta2=0.9)
    g_optimizer = tf.train.AdamOptimizer(lr2, beta1=0.5, beta2=0.9)
    ...
    discriminator_training_callback = ng.callbacks.SecondaryTrainer(
        ...
        var_list=d_vars,
        max_iters=1,# 'max_iters' times update D    1 time update G
